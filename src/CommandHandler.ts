...
class CommandHandler {
  ...
  constructor(
    ...
    if (dir) {
      ...
      client.on("message", (message) => {
        ...
        let embedMessages = instance.embedMessages;
        ...
        if (
          ...
          if (error) {
            ...
            if (embedMessages) {
              message.channel.send(this.errorEmbed(errorMsg));
            } else {
        ...
        if ((cooldown || globalCooldown) && user) {
          if (instance.botOwner.includes(message.author.id)) return;
        ...
        try {
          ...
          if (error) {
            ...
            if (embedMessages) {
              message.channel.send(
                this.errorEmbed(instance.messageHandler.get(guild, "EXCEPTION"))
              );
            } else {
  ...
  public errorEmbed(message: string) {
