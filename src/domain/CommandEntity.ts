...
export class CommandEntity {
  ...
  public get requiredRoles(): Collection<string, Role> | null {
    return (!this._requiredRoles || this._requiredRoles.size === 0) ? null : this._requiredRoles
  }
  ...
  public addRequiredRole({ role }: { role: Role }) {
    if (!this._requiredRoles?.find((v) => v.roleId === role.roleId)){
      // set it
      if (!this._requiredRoles) {
        this._requiredRoles = new Collection<string, Role>()
      }

      this._requiredRoles.set(role.roleId, role)
    }
  }

  public removeRequiredRole({ roleId }: { roleId: string }) {
    // TODO do we want to throw an error if the role didn't exist for the command?
    this._requiredRoles?.sweep(role => role.roleId === roleId)
    if (this._requiredRoles && this._requiredRoles.size === 0) {
